...
struct ARCASCClientTests {
    ...
    @Test("Returns stubbed builds for given app ID", .tags(.unit))
    func fetchBuildsReturnsStubbedBuilds() async throws {
        // Given
        let (sut, mock) = makeSUT()
        mock.stubbedBuilds = [
            try makeStubBuild(id: "build-1", version: "42"),
            try makeStubBuild(id: "build-2", version: "43")
        ]
    ...
    @Test("Propagates error from fetchBuilds", .tags(.unit))
    func fetchBuildsThrowsWhenErrorStubbed() async throws {
        // Given
        let (sut, mock) = makeSUT()
    ...
    @Test("Tracks submit call count and version ID", .tags(.unit))
    func submitForReviewTracksCall() async throws {
        // Given
        let (sut, mock) = makeSUT()
    ...
    @Test("Uploads metadata and tracks last upload", .tags(.unit))
    func uploadMetadataTracksCall() async throws {
        // Given
        let (sut, mock) = makeSUT()
